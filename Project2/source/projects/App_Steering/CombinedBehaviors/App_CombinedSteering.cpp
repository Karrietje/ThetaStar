...
void App_CombinedSteering::Start()
{
	...
	m_pFleeAgent = new SteeringAgent();
	m_pFleeAgent->SetSteeringBehavior(m_pBlendedSteering);
	m_pFleeAgent->SetMaxLinearSpeed(15.f);
	m_pFleeAgent->SetAutoOrient(true);
	m_pFleeAgent->SetMass(1.f);
	m_pFleeAgent->SetBodyColor({ 1,0,0 });
}
...
void App_CombinedSteering::Update(float deltaTime)
{
	...
	//UI
	{
		...
		//De sliders
		ImGui::SliderFloat("Seek", &m_pBlendedSteering->m_WeightedBehaviors[0].weight, 0.f, 1.f);
		ImGui::SliderFloat("Wander", &m_pBlendedSteering->m_WeightedBehaviors[1].weight, 0.f, 1.f);
	...
	fleeTarget.LinearVelocity = m_pDrunkAgent->GetLinearVelocity(); 
	fleeTarget.Position = m_pDrunkAgent->GetPosition();
	m_pFlee->SetTarget(fleeTarget);
	m_pFleeAgent->Update(deltaTime); 
	m_pFleeAgent->TrimToWorld(m_TrimWorldSize); 
...
void App_CombinedSteering::Render(float deltaTime) const
{
	...
	m_pFleeAgent->SetRenderBehavior(m_CanDebugRender);
	m_pFleeAgent->Render(deltaTime); 
